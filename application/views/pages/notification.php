        ...
        <div class="col-lg-12 col-md-12 mb-4">
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <table class="table table-striped table-bordered "  id="tbl_notif" style="width: 100%;" >
                <thead>
...
<!--         <div class="modal-footer">
  ...
  <!--Modal Form Login with Avatar Demo-->
   <div class="modal fade" id="Modal_read" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
     <div class="modal-dialog cascading-modal modal-avatar modal-sm" role="document">
       ...
       <!--   <div class="modal-header">
         ...
         <div class="modal-body text-center mb-1">
           <h5 class="mt-1 mb-2" id="msg">Your message here</h5></br>
           <h5 class="mt-1 mb-2" id="availabillity" hidden></h5>
           <div class="md-form ml-0 mr-0">
            
            <input type="text" id="notif_id" name="notif_id" hidden>
             
             <label for="form1" class="ml-0"></label>
           </div>
           <div class="text-center mt-4">
            <button class="btn btn-danger btn-sm" id="yes" >yes </button>
             <button class="btn btn-danger btn-sm" id="no" data-dismiss="modal" onClick="window.location.reload();">no
  ...
  <div class="modal fade" id="Modal_remove" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog cascading-modal modal-avatar modal-sm" role="document">
      <div class="modal-content">
        ...
        title="close"> x </button>
        <div class="modal-body text-center mb-1">
           <h5 class="mt-1 mb-2">Are you sure you want to remove this notification</h5>
          <div class="md-form ml-0 mr-0">
            <input type="text" id="notif_id" name="notif_id" hidden>
            <input  type="text" id="stud_id" name="stud_id" hidden>
          </div>
          <div class="text-center mt-4">
             <button class="btn btn-danger" id="remove" data-dismiss="modal">ok
             </button>
          </div>
        </div>
      </div>
 ...
 <!--  <script type="text/javascript" src="<?php echo base_url();?>mdbassets/js/jquery.gdocsviewer.js"></script>
  ...
  <script type="text/javascript">
    ...
    $('#tbl_notif').DataTable({
        autoWidth: false,
        destroy:true,
        'ajax':'<?php echo base_url('register/get_notif')?>'
    });
    ...
    function mark_as_read(msg, notif_id, availability){

      if(availability == 'available'){
        $('#Modal_read').modal('show');
        $('#msg').html('Fortunately there is ' +availability+ ' Scholarship, would you like to proceed for scholarship');
        $('#notif_id').val(notif_id);
        $('#availabillity').html(availability);
      }

      if(availability == 'unavailable'){

        $('#Modal_read').modal('show');
        $('#msg').html('Unfortunately there is ' +availability+ ' Scholarship, would you like to wait for a scholarship?');
        $('#notif_id').val(notif_id);
        $('#availabillity').html(availability);

        $('#yes').click(function(){
          $('#msg').html('Your request for waitlist enrollment status was already submitted');
          $('#yes').hide();
          $('#no').html('ok');
        })
      }
    ...
    $('#close').on('click',function(){
      ...
      $.ajax({
        url:'mark_msg_read',
        method:'POST',
        data:{mark_read:mark_read, notif_id:notif_id},
  ...
  function notif_remove(notif_id, student_id){
  ...
  $('#remove').on('click', function(){
    ...
    //$('#view_requirements').modal('hide');
    console.log(notif_id); console.log(stud_id);
    $.ajax({
      url:'remove_notif',
      method:'POST',
      data:{notif_id:notif_id, stud_id:stud_id
      },success:function(data){
        console.log(data);
        location.reload();
      }
